...
namespace Microsoft.Build.Utilities
{
        ...
        IMetadataContainer // expose direct underlying metadata for fast access in binary logger
    {
        ...
        public void WriteToStream(BinaryWriter writer)
        {
            ...
            foreach (KeyValuePair<string, string> kvp in EnumerateMetadataLazy())
            {
        ...
        public static void WriteArrayToStream(BinaryWriter writer, ITaskItem[] array)
        {
            if (array.Length == 0)
            {
                writer.Write((byte)0);
            ...
            {
                if (taskItem is not TaskItem item)
                {
        ...
        public void CreateFromStream(BinaryReader reader)
        {
            ...
            IEnumerable<KeyValuePair<string, string>> metadata = ReadMetadata(reader);
            (this as IMetadataContainer).ImportMetadata(metadata);

            IEnumerable<KeyValuePair<string, string>> ReadMetadata(BinaryReader reader)
            {
                int metadataCount = reader.ReadInt32();

                for (int i = 0; i < metadataCount; i++)
                {
                    yield return new(reader.ReadString(), reader.ReadString());
                }
            }
        }

        public static TaskItem[] CreateArrayFromStream(BinaryReader reader)
        {
            if (reader.ReadByte() == 0)
            {
                return [];
