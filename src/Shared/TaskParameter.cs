...
#nullable enable
...
#endif
        {
            ...
            {
                if (copyFrom is ITaskItem2 taskItem2)
                {
                    _escapedItemSpec = taskItem2.EvaluatedIncludeEscaped;
                    _escapedDefiningProject = taskItem2.GetMetadataValueEscaped(FileUtilities.ItemSpecModifiers.DefiningProjectFullPath);
#if !TASKHOST
                    if (taskItem2 is IMetadataContainer metadataContainer)
                    {
                        _customEscapedMetadata = CopyFromMetadataContainer(metadataContainer);
                    }
                    else
#endif
                    {
                        _customEscapedMetadata = CopyFromTaskItem2(taskItem2);
                    }
                }
                else
                {
                    ...
                    _customEscapedMetadata = CopyFromTaskItem(copyFrom);
                }

                ErrorUtilities.VerifyThrowInternalNull(_escapedItemSpec);

#if !TASKHOST
                static Dictionary<string, string> CopyFromMetadataContainer(IMetadataContainer metadataContainer)
                {
                    int metadataCount = 0;
                    foreach (KeyValuePair<string, string> entry in metadataContainer.EnumerateMetadata())
                    {
                        metadataCount++;
                    }

                    Dictionary<string, string> metadata = new(metadataCount, MSBuildNameIgnoreCaseComparer.Default);

                    foreach (KeyValuePair<string, string> entry in metadataContainer.EnumerateMetadata())
                    {
                        metadata[entry.Key] = EscapingUtilities.Escape(entry.Value) ?? string.Empty;
                    }
                ...
                static Dictionary<string, string> CopyFromTaskItem2(ITaskItem2 taskItem2)
                {
                    IDictionary clonedMetadata = taskItem2.CloneCustomMetadataEscaped();

                    if (clonedMetadata is Dictionary<string, string> metadata)
                    {
                        return metadata;
                    }

                    metadata = new Dictionary<string, string>(clonedMetadata.Count, MSBuildNameIgnoreCaseComparer.Default);
                ...
                static Dictionary<string, string> CopyFromTaskItem(ITaskItem taskItem)
                {
                    ...
                    foreach (DictionaryEntry entry in clonedMetadata)
                    {
                        metadata[(string)entry.Key] = EscapingUtilities.Escape((string?)entry.Value) ?? string.Empty;
                    }
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
            ...
            {
                get => EscapingUtilities.UnescapeAll(_escapedItemSpec);
            ...
            public ICollection MetadataNames
            {
                get
                {
                    List<string> metadataNames = [.. _customEscapedMetadata.Keys, .. FileUtilities.ItemSpecModifiers.All];
            ...
            public int MetadataCount => _customEscapedMetadata.Count + FileUtilities.ItemSpecModifiers.All.Length;
            ...
            public void RemoveMetadata(string metadataName)
            {
                ...
                _ = _customEscapedMetadata.Remove(metadataName);
            }
            ...
            public void CopyMetadataTo(ITaskItem destinationItem)
            {
                ...
                {
                    destinationItem.SetMetadata("OriginalItemSpec", EvaluatedIncludeEscaped);
                }
            }
            ...
            {
                Dictionary<string, string> clonedMetadata = new(_customEscapedMetadata.Count, MSBuildNameIgnoreCaseComparer.Default);
            ...
            public string GetMetadataValueEscaped(string metadataName)
            {
                ...
                {
                    _ = _customEscapedMetadata.TryGetValue(metadataName, out metadataValue);
                }
            ...
            /// </summary>
            public IDictionary CloneCustomMetadataEscaped() => new Dictionary<string, string>(_customEscapedMetadata, MSBuildNameIgnoreCaseComparer.Default);
            ...
            {
                if (_customEscapedMetadata.Count == 0)
                {
                    return [];
                }
            ...
            {
                foreach (KeyValuePair<string, string> kvp in _customEscapedMetadata)
                {
            ...
            public void Translate(ITranslator translator)
            {
                translator.InternPath(ref _escapedItemSpec, nullable: true);
                translator.InternPath(ref _escapedDefiningProject, nullable: true);
                translator.InternPathDictionary(ref _customEscapedMetadata, MSBuildNameIgnoreCaseComparer.Default);
