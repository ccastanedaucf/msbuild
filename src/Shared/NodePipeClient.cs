...
namespace Microsoft.Build.Internal
{
    internal sealed class NodePipeClient : NodePipeBase
    {
        ...
        private readonly int[] _handshakeComponents;
        ...
        {
            _pipeName = pipeName;
            _handshakeComponents = handshake.RetrieveHandshakeComponents();
...
#pragma warning restore SA1111, SA1009 // Closing parenthesis should be on line of last parameter
        ...
        protected override Stream NodeStream => _pipeClient;

        internal void ConnectToServer(int timeout)
        {
            CommunicationsUtilities.Trace("Attempting connect to pipe {0} with timeout {1} ms", _pipeName, timeout);
        ...
        {
            for (int i = 0; i < _handshakeComponents.Length; i++)
            {
                CommunicationsUtilities.Trace("Writing handshake part {0} ({1}) to pipe {2}", i, _handshakeComponents[i], _pipeName);
                _pipeClient.WriteIntForHandshake(_handshakeComponents[i]);
            }
