                    ...
                    foreach (ITaskItem output in outputs)
                    {
                        ...
                        if (output != null)
                        {
                            ...
                            if (output is TaskItem outputAsProjectItem)
                            {
                                ...
                                newItem.SetMetadata(outputAsProjectItem); // copy-on-write!
                                // newItem.SetMetadata(outputAsProjectItem); // copy-on-write!
                            }
                            else if (output is ITaskItem2 outputAsITaskItem2)
                            {
                                ...
                                if (output is IMetadataContainer metadataContainer
                                    && outputAsITaskItem2.CloneCustomMetadataEscaped() is CopyOnWriteDictionary<string, ProjectMetadataInstance> copyOnWriteDictionary)
                                {
                                    CopyOnWritePropertyDictionary<ProjectMetadataInstance> copyOnWritePropertyDictionary = new(copyOnWriteDictionary.ToImmutableDictionary());
                                    newItem.SetMetadata(copyOnWritePropertyDictionary);
                                }
                                else
                                {
